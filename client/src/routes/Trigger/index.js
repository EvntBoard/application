...
export default () => {
  ...
  const onSubmitFormTrigger = async (data) => {
    window.app.trigger.update(data).then((result) => {
  ...
  const onClickCreate = () => {
    window.app.trigger.create({...SAMPLE_TRIGGER, }).then((result) => {
      setTriggers([
        ...triggers,
        result
      ])
  ...
  const onDeleteTrigger = (data) => {
    window.app.trigger.delete(data).then((result) => {
      if (result) {
  ...
  const onDupplicateTrigger = (data) => {
    window.app.trigger.create({...data, id: null, name: `DUP-${data.name}` }).then((result) => {
      setTriggers([
        ...triggers,
        result
      ])
    ...
    <div className='trigger app-content'>
      <div className='list'>
        <div className='search'>
          <InputGroup value={search} onChange={onChangeSearch} placeholder='Recherche ...' />
        </div>
        <div className='items'>
        ...
        <div className='actions'>
          <Button fill onClick={onClickCreate}>Créer un trigger</Button>
        </div>
      </div>
      <div className='form'>
        {
